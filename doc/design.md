...
- `site_bucket_name` / `artifact_bucket_name` / `cloudfront_logs_bucket_name` — S3 バケット名はアカウント内で一意である必要がある。`site_bucket_name` と `artifact_bucket_name` は tfvars、`cloudfront_logs_bucket_name` はモジュール内部で派生するが、命名規則を満たすか確認。
